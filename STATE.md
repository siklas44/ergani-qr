...
## 🆕 Πρόσφατο (2026-06-28)
- Fix: το dropdown καταστήματος έδειχνε «— Επίλεξε κατάστημα —» ενώ ήταν
  επιλεγμένο κατάστημα· η `setCurrentStore()` συγχρονίζει πλέον το
  `storeSelector.value` (single authority) + belt-and-braces στη
  `renderStoreSelector()`.

## 🆕 Πρόσφατο (2026-06-27)
- 👥 Χρήστες: ο κωδικός νέου χρήστη εμφανίζεται πλέον με κουμπί «📋 Αντιγραφή» και
  panel επιβεβαίωσης μετά τη δημιουργία («δεν εμφανίζεται ξανά»)· στο edit μπήκε
  «📧 Αποστολή e-mail ορισμού κωδικού» (ο κωδικός δεν αλλάζει από την εφαρμογή —
  client-only, χωρίς Admin SDK)· η διαγραφή λέει ρητά ότι ο λογαριασμός σύνδεσης
  παραμένει στο Firebase Auth.
